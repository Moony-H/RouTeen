...
class SlideUpLayout: ViewGroup {
    ...
    constructor(context: Context, attrs: AttributeSet?, style: Int) : super(context, attrs, style) {
        ...
        attrs?.let {
            val typeArray = context.obtainStyledAttributes(attrs, R.styleable.SlideUpLayout)
            mSlideLayoutExposureHeight = typeArray.getDimensionPixelSize(
                R.styleable.SlideUpLayout_moony_SlideUpLayout_exposureHeight,
                164
            )
            mSlideLayoutMaxHeight = typeArray.getDimensionPixelOffset(
                R.styleable.SlideUpLayout_moony_SlideUpLayout_maxHeight,
                1500
            )
            mSlideLayoutShadowColor =
                typeArray.getColor(R.styleable.SlideUpLayout_moony_SlideUpLayout_color, -1)
            mScrollViewResId =
                typeArray.getResourceId(
                    R.styleable.SlideUpLayout_moony_SlideUpLayout_scrollViewId,
                    -1
                )
            slideEnabled =
                typeArray.getBoolean(
                    R.styleable.SlideUpLayout_moony_SlideUpLayout_slideEnabled,
                    true
                )
            typeArray.recycle()
        }
